...
where `[SiteId]` is the site identifier stored in the `currSiteId` variable at the beginning of `runAnalyasis()`; `[i]` is an index enumerating the various result files generated by your analytic workflow (this can be numeric or alpha,  multiple characters); `[extension]` is a file extension.  For example, if the analysis is being run at a site called "Harvard", and it produces two files, one with a table of summary results, and another with a set of model coefficients, then the files might be named as: `Harvard_SummaryTable.csv` and `Harvard_Coefficients.txt`.  `submitAnalysis()` will pick up both of these and put them in a branch of the `Phase2.1[PROJECT_NAME]RSummariesPublic` called `topic-Harvard`.

In the `Phase2.1MyAnalysisRPackage` example, for simplicity we have commented out code that reads site level data in `runAnalysis()` and hard-coded a site ID.  We then installed the package and ran all three `runAnalysis()`, `validateAnalysis()`, and `submitAnalysis()` functions, which pushed dummy result files to a branch in `Phase2.1MyAnalysisRSummariesPublic`.

For sites that need to run their analyses in isloated (no internet connectivity) environments, the container should be saved (see [README](https://github.com/covidclinical/Phase2.1DockerAnalysis) for the container) after the user runs `runAnalsyis()` and `validateAnalysis()`, but before running `submitAnalysis()`.  The default recommended location (see above) for the results of `runAnalsyis()` to be saved is local to the container's file system, so the result files will persist while the image is moved to a host with internet connectivity (but no access to sensitive patient-level site data) where the summary data can be pushed to the GitHub repository.
